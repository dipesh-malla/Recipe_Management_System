...
const staticNotifications: Notification[] = [
    {
        ...
        verb: "liked",
        object: { type: "recipe", id: 1, content: "Classic Spaghetti Carbonara" },
        isRead: false,
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            ...
            try {
                ...
                setError(`Failed to fetch notifications: ${err.message}. Displaying static data.`);
                setNotifications(staticNotifications);
            }
        };
        fetchNotifications();
        ...
        <MainLayout>
            <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="flex justify-between items-center mb-8">
                    ...
                    <Button variant="outline" size="sm" onClick={handleMarkAllAsRead} disabled={!currentUser || notifications.every(n => n.isRead)}>
                            ...
                            <li key={notification.id} onClick={() => !notification.isRead && handleMarkAsRead(notification.id)} className={`p-4 flex items-start gap-4 ${!notification.isRead ? 'bg-orange-50/50 cursor-pointer' : 'bg-white'}`}>
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-2">
                                        <img src={notification.actor.profile.url} alt={notification.actor.displayName} className="h-10 w-10 rounded-full object-cover" />
                                        <p className="text-gray-700">
                                            <Link to={`/user/${notification.actor.id}`} className="font-bold hover:underline">{notification.actor.displayName}</Link>
                                            {` ${notification.verb} `}
                                            {notification.object.type !== 'user' && 
